...
// .wrangler/tmp/bundle-TbOK11/checked-fetch.js
...
// .wrangler/tmp/bundle-TbOK11/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-3BvqFq/functionsWorker-0.34276981216804714.mjs
...
function formatGpsIso6709(lat2, lon2, alt2) {
  const latSign = lat2 >= 0 ? "+" : "-";
  const lonSign = lon2 >= 0 ? "+" : "-";
  const latStr = `${latSign}${Math.abs(lat2).toFixed(4)}`;
  const lonStr = `${lonSign}${Math.abs(lon2).toFixed(4).padStart(9, "0")}`;
  const altStr = `+${Math.floor(alt2).toString().padStart(3, "0")}`;
...
function injectVideoMetadata(videoData, metadata) {
  ...
  return insertUdtaIntoMoov(data, moovAtom, udtaAtom);
}
...
function buildUdtaAtom(meta) {
  const content = [];
  const metaAtom = buildMetaAtom(meta);
...
__name2(buildUdtaAtom, "buildUdtaAtom");
...
__name(buildMetaAtom, "buildMetaAtom");
__name2(buildMetaAtom, "buildMetaAtom");
...
async function onRequestPost(context) {
  try {
    ...
    const city2 = formData.get("city") || "New York";
    const device2 = formData.get("device") || "iPhone 16 Pro";
    if (!photo) {
    ...
    const metadata = generateMetadata(city2, device2);
...
// .wrangler/tmp/bundle-TbOK11/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-TbOK11/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.34276981216804714.js.map
